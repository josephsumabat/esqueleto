...
instance Semigroup LockingClause where
  ...
  -- Cases of comparing general to postgres specific locks.
  -- ForUpdate/ForUpdateSkipLocked > Postgres For Update Of > ForShare/LockInShareMode > PostgresFor Share Of if comparing between postgres and non-postgres locks
  (<>) l@(GeneralLockingClause ForUpdate) (PostgresLockingClause _) = l
  (<>) _ r@(GeneralLockingClause ForUpdate) = r
  (<>) l@(GeneralLockingClause ForUpdateSkipLocked) (PostgresLockingClause _) = l
  (<>) _ r@(GeneralLockingClause ForUpdateSkipLocked) = r
  (<>)  l@(PostgresLockingClause (PostgresLockingKind PostgresForUpdate _ _)) (GeneralLockingClause ForShare)= l
  (<>) (GeneralLockingClause ForShare) r@(PostgresLockingClause (PostgresLockingKind PostgresForUpdate _ _)) = r
  (<>)  l@(PostgresLockingClause (PostgresLockingKind PostgresForUpdate _  _)) (GeneralLockingClause LockInShareMode)= l
  (<>) (GeneralLockingClause LockInShareMode) r@(PostgresLockingClause (PostgresLockingKind PostgresForUpdate  _ _)) = r
  (<>) l@(GeneralLockingClause ForShare) _ = l
  (<>)  _ r@(GeneralLockingClause ForShare) = r
  (<>) l@(GeneralLockingClause LockInShareMode) _ = l
  (<>)  _ r@(GeneralLockingClause LockInShareMode)= r
